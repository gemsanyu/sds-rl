...
from env.sa_fcfs import StateAwareFCFSScheduler

class StateAwareBFScheduler(StateAwareFCFSScheduler):
    def __init__(self, simulator: SimulatorHandler, host_monitor: HostMonitor) -> None:
        super().__init__(simulator, host_monitor)
        # every submission, try to schedule
    ...
    def backfill(self) -> None:
        ...
        next_releases = sorted(self.simulator.agenda,
                               key=lambda a: a.release_time)
        for agenda in next_releases:
            print(agenda.release_time, agenda.host)
